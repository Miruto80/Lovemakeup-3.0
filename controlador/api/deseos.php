...
try {
    $listaDeseoModel = new ListaDeseo();
    $method = $_SERVER['REQUEST_METHOD'];

    // 1. ENDPOINT PARA OBTENER LA LISTA (GET)
    if ($method === 'GET') {
        $cedula = $_GET['cedula'] ?? null;

        if (!$cedula) {
            http_response_code(400);
            echo json_encode(['status' => 'error', 'message' => 'Falta el parámetro cedula']);
            exit;
        }

        $resultado = $listaDeseoModel->obtenerListaDeseo($cedula);
        echo json_encode(['status' => 'success', 'data' => $resultado]);
        exit;
    }

    // 2. ENDPOINTS PARA AGREGAR, ELIMINAR, VACIAR Y CONSULTAR ESTADO (POST)
    if ($method === 'POST') {
        ...
        if ($operacion === 'esta_en_lista') {
            $cedula = $params['id_persona'] ?? null; 
            $id_producto = $params['id_producto'] ?? null;

            if (!$cedula || !$id_producto) {
                http_response_code(400);
                echo json_encode(['status' => 'error', 'message' => 'Datos incompletos para verificar']);
                exit;
            }

           
            $existe = $listaDeseoModel->estaEnLista($cedula, $id_producto);
            echo json_encode(['status' => 'success', 'existe' => $existe]);
            exit;
        }

        
        if (in_array($operacion, ['agregar', 'eliminar', 'vaciar'])) {
            $resultado = $listaDeseoModel->procesarListaDeseo($jsonInput);
        ...
        http_response_code(400);
        echo json_encode(['status' => 'error', 'message' => 'Operación POST no válida o no especificada']);
        exit;
    }
