$(document).ready(function () {
  ...
  function activarLoaderBoton(idBoton, texto) {
    ...
    $boton.html(
      `<span class="spinner-border spinner-border-sm me-2" role="status"></span>${texto}`
    );
  }
  ...
  function validarReferenciaBancaria(input) {
    ...
    valido
      ? limpiarError(input)
      : mostrarError(input, "Debe tener entre 4 y 6 dígitos.");
    return valido;
  }
  ...
  function validarTelefonoEmisor(input) {
    ...
    valido
      ? limpiarError(input)
      : mostrarError(input, "Formato válido: 04141234567");
    return valido;
  }
  ...
  $("#referencia_bancaria").on("input", () =>
    validarReferenciaBancaria($("#referencia_bancaria"))
  );
  $("#telefono_emisor").on("input", () =>
  ...
  $("#btn-guardar-pago").on("click", function (e) {
    ...
    if (
      ...
      return Swal.fire(
        "Error",
        "Ingrese una referencia bancaria válida",
        "warning"
      );
    }
    if (
      ...
      return Swal.fire(
        "Error",
        "Ingrese un teléfono del emisor válido",
        "warning"
      );
    }
    if (!$("#banco").val()) {
    ...
    // ACTIVAR loader antes del SweetAlert
    activarLoaderBoton("#btn-guardar-pago", "Procesando...");
        ...
        success(res) {
          ...
          if (res.success) {
            Swal.fire({
              ...
              setTimeout(
                () => (window.location.href = "?pagina=Pedidoconfirmar"),
                1000
              )
            );
          } else {
