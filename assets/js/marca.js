$(function() {
  ...
  $('#btnAbrirRegistrar').click(()=> {
    ...
    $('#modalTitleText').text('Registrar marca');
  ...
  window.abrirModalModificar = function() {
    ...
    $('#modalTitleText').text('Modificar marca');
  ...
  function enviaAjax(fd) {
    ...
    .done(res=>{
      if (res.accion==='incluir'    && res.respuesta==1) return mensajeOK('Marca registrada');
      if (res.accion==='actualizar' && res.respuesta==1) return mensajeOK('Marca modificada');
      if (res.accion==='eliminar'   && res.respuesta==1) return mensajeOK('Marca eliminada');
  ...
  const steps = [
    {
      ...
      popover: {
        ...
        description: 'Aquí ves todas las marcas activas.',
        side: 'left'
      }
    },
    {
      ...
      popover: {
        ...
        description: 'Abre el modal para crear una nueva marca.',
        side: 'bottom'
      }
    },
    {
      ...
      popover: {
        title: 'Editar marca',
        description: 'Haz clic aquí para modificar una marca existente.',
        side: 'left'
      }
    },
    {
      ...
      popover: {
        title: 'Eliminar marca',
        description: 'Este botón elimina la marca seleccionada.',
        side: 'left'
      }
    },
    {
      popover: {
        title: '¡Listo!',
        description: 'Has terminado la guía de marcas.'
      }
    }
  ];
