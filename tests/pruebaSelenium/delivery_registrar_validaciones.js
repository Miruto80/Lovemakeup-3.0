...
const TEST_CASE_EXTERNAL_ID = 'Prueba-44';
...
// === TEST AUTOMATIZADO: ERROR AL REGISTRAR DELIVERY POR INCUMPLIMIENTO DEL RANGO PERMITIDO DE CARACTERES ===
async function runTest() {
  ...
  try {
    ...
    testSteps.push('Llenar datos con valores fuera de rango (nombre muy corto: "Rh", teléfono muy corto: "042450719")');
    ...
    // Teléfono muy corto (042450719 - menos de 12 caracteres)
    ...
    await contactoInput.sendKeys('042450719');
    await driver.sleep(500);
    ...
    try {
      ...
      if (errorNombreText || errorContactoText) {
        notes = 'Error de validación detectado correctamente. Nombre: ' + errorNombreText + ', Contacto: ' + errorContactoText + '. Se validó que se debe cumplir el rango permitido de caracteres.';
        status = 'p';
      } else {
        try {
          ...
          if (alertText.includes('obligatorios') || alertText.includes('error') || alertText.includes('caracteres')) {
    ...
    console.log('Error de validación de rango de caracteres verificado exitosamente.');
