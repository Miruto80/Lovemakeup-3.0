...
const TEST_CASE_EXTERNAL_ID = 'Prueba-35';
...
// === TEST AUTOMATIZADO: ERROR AL INTENTAR MODIFICAR EL TIPO USUARIO PRINCIPAL DEL SISTEMA ===
async function runTest() {
  ...
  try {
    ...
    // === Paso 3: Seleccionar el tipo usuario Administrador (id=3) y hacer click en Modificar ===
    testSteps.push('Seleccionar el tipo usuario Administrador y hacer click en Modificar');
    console.log('Buscando botón de modificar del Administrador...');
    
    // Buscar el botón de modificar del Administrador (id=3)
    const modificarButtons = await driver.findElements(By.css('.modificar'));
    let adminModificarBtn = null;
    
    for (const btn of modificarButtons) {
      const dataId = await btn.getAttribute('data-id');
      if (dataId === '3') {
        adminModificarBtn = btn;
        break;
      }
    }
    
    if (!adminModificarBtn) {
      throw new Error('No se encontró el botón de modificar del Administrador');
    }
    
    await driver.executeScript("arguments[0].scrollIntoView({block: 'center', behavior: 'smooth'});", adminModificarBtn);
    await driver.sleep(500);
    await driver.wait(until.elementIsVisible(adminModificarBtn), 10000);
    await adminModificarBtn.click();
    await driver.sleep(2000);

    // === Paso 4: Verificar mensaje de error (el modal no debería abrirse) ===
    testSteps.push('Verificar mensaje de error');
    console.log('Verificando mensaje de error...');
    
    try {
      // El sistema debería mostrar un mensaje de error antes de abrir el modal
      ...
      if (errorText.includes('Administrador') && (errorText.includes('no puede modificarse') || errorText.includes('permitida') || errorText.includes('no puede'))) {
        console.log('Mensaje de error detectado correctamente: ' + errorText);
        testSteps.push('Mensaje de error confirmado: ' + errorText);
        notes = 'Error detectado correctamente: ' + errorText + '. Se validó que el rol Administrador no puede modificarse.';
        status = 'p';
      } else {
        const htmlContainer = await driver.findElement(By.css('.swal2-html-container'));
        const htmlText = await htmlContainer.getText();
        if (htmlText.includes('Administrador') && (htmlText.includes('no puede modificarse') || htmlText.includes('permitida') || htmlText.includes('no puede'))) {
          console.log('Mensaje de error detectado en HTML: ' + htmlText);
          testSteps.push('Mensaje de error confirmado: ' + htmlText);
          notes = 'Error detectado correctamente: ' + htmlText + '. Se validó que el rol Administrador no puede modificarse.';
          status = 'p';
        } else {
          // Verificar si el modal se abrió (no debería)
          try {
            const modal = await driver.findElement(By.id('modificar'));
            const isModalVisible = await modal.getAttribute('class');
            if (isModalVisible.includes('show')) {
              throw new Error('El modal se abrió cuando no debería. El Administrador no debería poder modificarse.');
            }
          } catch (e) {
            // El modal no se abrió, lo cual es correcto
            console.log('Modal no se abrió (comportamiento esperado)');
          }
      ...
      try {
        ...
        // El modal no se abrió, lo cual es correcto, pero necesitamos el mensaje de error
        throw new Error('No se pudo verificar el mensaje de error. Error: ' + e.message);
      }
    }

    console.log('Error al intentar modificar Administrador verificado exitosamente.');
