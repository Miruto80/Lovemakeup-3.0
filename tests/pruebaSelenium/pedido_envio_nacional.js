...
const TEST_CASE_EXTERNAL_ID = 'Prueba-99';
...
// === TEST AUTOMATIZADO: REALIZAR PEDIDO - ENVÍO NACIONAL ===
async function runTest() {
  ...
  try {
    ...
    // === Seleccionar Envíos Nacionales ===
    testSteps.push('Hacer click en Envíos Nacionales');
    console.log('Seleccionando Envíos Nacionales...');
    const envioNacionalRadio = await driver.findElement(By.id('op2'));
    await driver.wait(until.elementIsVisible(envioNacionalRadio), 10000);
    await driver.executeScript("arguments[0].click();", envioNacionalRadio);
    await driver.sleep(1500);
    testSteps.push('Modal con datos a ingresar desplegado');

    // === Seleccionar Empresa ===
    testSteps.push('Seleccionar Empresa');
    console.log('Seleccionando empresa...');
    const empresaSelect = await driver.findElement(By.css('select[name="empresa_envio"]'));
    await driver.wait(until.elementIsVisible(empresaSelect), 10000);
    ...
    await driver.sleep(500);
    testSteps.push('Opciones de empresas de envíos nacionales desplegadas');

    // === Ingresar código de sucursal ===
    testSteps.push('Ingresar código de sucursal');
    console.log('Ingresando código de sucursal...');
    const codigoSucursal = await driver.findElement(By.id('codigoSucursal'));
    await driver.wait(until.elementIsVisible(codigoSucursal), 10000);
    await codigoSucursal.clear();
    await codigoSucursal.sendKeys('12345');
    await driver.sleep(500);

    // === Ingresar nombre de sucursal ===
    testSteps.push('Ingresar nombre de sucursal');
    console.log('Ingresando nombre de sucursal...');
    const nombreSucursal = await driver.findElement(By.id('nombreSucursal'));
    await driver.wait(until.elementIsVisible(nombreSucursal), 10000);
    await nombreSucursal.clear();
    await nombreSucursal.sendKeys('MRW sucursal Oeste');
    await driver.sleep(500);
    ...
    // Completar pago (similar a tienda física)
    ...
    console.log('Pedido con envío nacional realizado exitosamente.');
    notes = 'Pedido realizado exitosamente. Tipo de entrega: Envío Nacional. Empresa seleccionada, código y nombre de sucursal ingresados correctamente.';
    status = 'p';
