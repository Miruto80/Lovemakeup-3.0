...
const TEST_CASE_EXTERNAL_ID = 'Prueba-79';
...
// === TEST AUTOMATIZADO: MODIFICAR DATOS DEL CLIENTE - ESTATUS INACTIVO ===
async function runTest() {
  ...
  try {
    ...
    // === Paso 5: Cambiar estatus a Inactivo ===
    testSteps.push('Cambiar estatus a Inactivo');
    console.log('Modificando estatus...');

    const estatusSelect = await driver.findElement(By.css('select[name="estatus"]'));
    await driver.wait(until.elementIsVisible(estatusSelect), 10000);
    await estatusSelect.click();
    await driver.sleep(300);
    await estatusSelect.sendKeys(Key.ARROW_DOWN); // Seleccionar "Inactivo" (value="2")
    ...
    // === Paso 9: Buscar el cliente y verificar estatus Inactivo ===
    testSteps.push('Buscar el cliente y verificar estatus Inactivo');
    console.log('Buscando cliente modificado...');
    await driver.sleep(2000);
    
    // Buscar el cliente en la tabla y verificar que aparece como Inactivo
    const estatusElements = await driver.findElements(By.css('.badge.bg-danger'));
    if (estatusElements.length > 0) {
      const estatusText = await estatusElements[0].getText();
      if (estatusText.includes('Inactivo')) {
        console.log('Estatus Inactivo verificado correctamente en la tabla.');
        testSteps.push('Estatus Inactivo verificado en la tabla');
      }
    }

    console.log('Cliente modificado a Inactivo exitosamente.');
    notes = 'Cliente modificado exitosamente. Estatus cambiado a Inactivo';
    status = 'p';
