...
const TEST_CASE_EXTERNAL_ID = 'Prueba-67';
...
// === TEST AUTOMATIZADO: ERROR AL MODIFICAR CATEGORIA CON NOMBRE YA REGISTRADO ===
async function runTest() {
  ...
  try {
    ...
    // === Paso 3: Obtener nombre de otra categoria existente ===
    testSteps.push('Obtener nombre de otra categoria existente');
    console.log('Buscando otra categoria existente...');
    
    // Buscar la segunda categoria en la tabla (si existe) para obtener su nombre
    ...
    try {
      ...
      if (filas.length >= 2) {
        ...
        if (celdas.length > 0) {
          ...
          console.log('Otra categoria existente encontrada: ' + nombreExistente);
        }
      }
    } catch (e) {
      console.log('No se pudo obtener otra categoria existente, usando: ' + nombreExistente);
    }

    // === Paso 4: Seleccionar una categoria y hacer click en Modificar ===
    ...
    // === Paso 6: Modificar con nombre existente ===
    testSteps.push('Modificar con nombre existente de otra categoria: ' + nombreExistente);
    console.log('Modificando con nombre existente...');

    // Cambiar nombre a uno existente
    ...
    try {
      ...
      if (errorText.includes('existe') || errorText.includes('registrado') || errorText.includes('mismo nombre')) {
        ...
        notes = 'Error detectado correctamente: ' + errorText + '. Se validó que no se puede modificar una categoria asignándole un nombre ya registrado.';
        status = 'p';
      } else {
        ...
        if (htmlText.includes('existe') || htmlText.includes('registrado') || htmlText.includes('mismo nombre')) {
          ...
          notes = 'Error detectado correctamente: ' + htmlText + '. Se validó que no se puede modificar una categoria asignándole un nombre ya registrado.';
          status = 'p';
        } else {
