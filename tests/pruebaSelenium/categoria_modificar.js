...
// === TEST AUTOMATIZADO: MODIFICAR CATEGORIA ===
async function runTest() {
  ...
  try {
    ...
    testSteps.push('Modificar los datos de la categoria');
    console.log('Modificando datos...');

    const timestamp = Date.now();
    const nuevoNombre = `Iluminador${String(timestamp).substring(7, 11)}`;
    ...
    console.log('Datos de la categoria modificados.');
    testSteps.push('Datos de la categoria modificados');
    ...
    try {
      ...
      if (successText.includes('Categoria modificada') || successText.includes('éxito') || successText.includes('actualizado')) {
      ...
      try {
        ...
        if (await tabla.isDisplayed()) {
          console.log('Tabla visible - Categoria modificada exitosamente.');
        } else {
    ...
    notes = 'Categoria modificada exitosamente. Nuevo nombre: ' + nuevoNombre;
    status = 'p';
