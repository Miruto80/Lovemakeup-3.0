...
const TEST_CASE_EXTERNAL_ID = 'Prueba-24';
...
async function runTest() {
  ...
  try {
    ...
    // === Paso 3: Buscar producto para eliminar ===
    testSteps.push('Buscar producto para eliminar');
    console.log('Buscando producto en la tabla...');
    
    await driver.sleep(2000);
    
    // Buscar la primera fila con botón de eliminar habilitado
    ...
    try {
      ...
      for (const fila of filas) {
        try {
          const btn = await fila.findElement(By.css('button.eliminar'));
          ...
          if (isEnabled) {
            ...
            console.log(`Producto encontrado para eliminar: ${nombreProducto}`);
      ...
      if (!botonEliminar) {
        throw new Error('No se encontró ningún producto con botón de eliminar habilitado');
      }
    } catch (e) {
      throw new Error('No se pudo encontrar un producto para eliminar: ' + e.message);
    }

    // === Paso 4: Hacer clic en botón de eliminar ===
    testSteps.push('Hacer clic en botón de eliminar');
    console.log('Haciendo clic en botón de eliminar...');
    await driver.executeScript("arguments[0].scrollIntoView({block: 'center', behavior: 'smooth'});", botonEliminar);
    await driver.sleep(500);
    await botonEliminar.click();
    await driver.sleep(2000);

    // === Paso 5: Confirmar eliminación ===
    testSteps.push('Confirmar eliminación en alerta');
    console.log('Esperando alerta de confirmación...');
    
    await driver.sleep(2000);
    
    // Buscar el botón de confirmación "Sí, eliminar"
    let confirmButton = null;
    try {
      const buttons = await driver.findElements(By.css('button'));
      for (const btn of buttons) {
        const text = await btn.getText();
        if (text.includes('eliminar') || text.includes('Eliminar') || text.includes('Sí')) {
          confirmButton = btn;
          break;
        }
      }
      ...
      try {
        ...
        throw new Error('No se pudo confirmar la eliminación: ' + e.message);
      }
    }

    // === Paso 6: Verificar eliminación exitosa ===
    testSteps.push('Verificar eliminación exitosa');
    ...
    if (successFound || pageReloaded) {
      ...
      testSteps.push('Producto eliminado exitosamente');
      status = 'p';
      notes = `Prueba completada exitosamente. El producto fue eliminado correctamente: ${nombreProducto}`;
    } else {
      ...
      if (errorFound) {
        ...
        if (pageReloaded) {
          console.log('Producto eliminado (página recargada).');
          testSteps.push('Producto eliminado exitosamente');
          status = 'p';
          notes = `Prueba completada. El producto fue eliminado: ${nombreProducto}`;
        } else {
          throw new Error('No se pudo confirmar la eliminación del producto');
        }
      }
    }
