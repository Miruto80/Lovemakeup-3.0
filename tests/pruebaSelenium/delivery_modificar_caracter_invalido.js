...
const TEST_CASE_EXTERNAL_ID = 'Prueba-43';
...
// === TEST AUTOMATIZADO: ERROR AL MODIFICAR DELIVERY CON CARÁCTER INVÁLIDO ===
async function runTest() {
  ...
  try {
    ...
    testSteps.push('Modificar con caracteres inválidos (nombre con número, teléfono con letra)');
    console.log('Modificando con caracteres inválidos...');

    // Nombre con número (Rh3chard Virguez)
    const nombreInput = await driver.findElement(By.id('nombre'));
    await driver.wait(until.elementIsVisible(nombreInput), 10000);
    await nombreInput.clear();
    await nombreInput.sendKeys('Rh3chard Virguez');
    await driver.sleep(500);

    // Teléfono con letra O (0424507195O)
    ...
    await contactoInput.sendKeys('0424507195O');
    await driver.sleep(500);
