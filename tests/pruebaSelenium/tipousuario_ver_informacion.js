...
const TEST_CASE_EXTERNAL_ID = 'Prueba-30';
...
async function runTest() {
  ...
  try {
    ...
    await driver.wait(until.elementLocated(By.css('button[data-bs-target="#infoModal"]')), 15000);
    console.log('Modulo Tipo Usuario cargado correctamente.');
    testSteps.push('Módulo Tipo Usuario cargado correctamente');

    // === Paso 3: Seleccionar un tipo usuario y hacer click en Ver información ===
    testSteps.push('Seleccionar un tipo usuario y hacer click en Ver información');
    console.log('Buscando botón de ver información...');
    
    const verInfoButtons = await driver.findElements(By.css('button[data-bs-target="#infoModal"]'));
    ...
    testSteps.push('Verificar que el modal muestra la información del tipo de usuario');
    console.log('Verificando modal de información...');
    
    await driver.wait(until.elementLocated(By.id('infoModal')), 10000);
    const modal = await driver.findElement(By.id('infoModal'));
    ...
    if (modalText && modalText.length > 0) {
      ...
      testSteps.push('Modal de información abierto y muestra datos del tipo de usuario');
    } else {
      throw new Error('El modal no muestra información del tipo de usuario');
    }

    console.log('Información del tipo de usuario mostrada exitosamente.');
    notes = 'Información del tipo de usuario mostrada exitosamente en el modal.';
    status = 'p';
