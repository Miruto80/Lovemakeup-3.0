...
const TEST_CASE_EXTERNAL_ID = 'Prueba-80';
...
// === TEST AUTOMATIZADO: ELIMINAR CUENTA DEL CLIENTE (Actualizar datos) ===
async function runTest() {
  ...
  try {
    ...
    // === Paso 3: Modificar nombre y apellido ===
    testSteps.push('Modificar nombre y apellido');
    ...
    await nombreInput.sendKeys('america');
    ...
    await apellidoInput.sendKeys('yepez');
    await driver.sleep(500);

    // === Paso 4: Hacer click en Actualizar datos ===
    ...
    try {
      ...
      if (alertText.includes('guardar') || alertText.includes('Desea') || alertText.includes('cambios')) {
        await alert.accept();
        await driver.sleep(3000);
      } else {
        await alert.dismiss();
    ...
    try {
      ...
      if (successText.includes('Actualizado') || successText.includes('éxito') || successText.includes('exito')) {
      ...
      const nombreActualizado = await driver.findElement(By.id('nombre'));
      const nombreValue = await nombreActualizado.getAttribute('value');
      if (nombreValue.includes('america')) {
    ...
    console.log('Datos actualizados exitosamente.');
    notes = 'Datos actualizados exitosamente. Nombre: america, Apellido: yepez';
    status = 'p';
