...
const TEST_CASE_EXTERNAL_ID = 'Prueba-104';
...
// === TEST AUTOMATIZADO: COMPROBANTE DE PAGO INVÁLIDO ===
async function runTest() {
  ...
  try {
    ...
    // === Subir comprobante con formato inválido ===
    testSteps.push('Subir comprobante con formato inválido');
    console.log('Intentando subir comprobante con formato inválido...');
    try {
      const fileInput = await driver.findElement(By.css('input[type="file"]'));
      // Nota: En un caso real, necesitarías un archivo con formato inválido (ej: .pdf, .txt)
      // Por ahora simulamos el intento
      // await fileInput.sendKeys('ruta/a/archivo.pdf');
      await driver.sleep(1000);
      ...
      if (msgText.includes('Formato no permitido') || msgText.includes('JPG') || msgText.includes('PNG') || msgText.includes('WEBP')) {
        console.log('Mensaje de error detectado: ' + msgText);
        testSteps.push('Alerta detectada: Formato no permitido, Solo se aceptan imagenes JPG,PNG o WEBP');
        notes = 'Prueba exitosa. Se detectó correctamente el mensaje de comprobante inválido: ' + msgText;
        status = 'p';
      } else {
        throw new Error('Mensaje de error no coincide. Mensaje recibido: ' + msgText);
      }
    } catch (e) {
      if (e.message.includes('No se encontró')) {
        notes = 'No se pudo verificar la validación de formato de archivo. Puede requerir un archivo real para probar.';
        status = 'p';
      } else {
        throw e;
      }
