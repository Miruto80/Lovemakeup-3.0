...
const TEST_CASE_EXTERNAL_ID = 'Prueba-78';
...
// === TEST AUTOMATIZADO: MODIFICAR DATOS DEL CLIENTE - CORREO REGISTRADO ===
async function runTest() {
  ...
  try {
    ...
    // === Paso 5: Modificar correo a uno ya registrado ===
    testSteps.push('Modificar correo a uno ya registrado');
    console.log('Modificando correo...');
    ...
    await correoInput.sendKeys('jefe@gmail.com'); // Correo ya registrado
    await driver.sleep(500);
    ...
    testSteps.push('Verificar mensaje de error de correo registrado');
    ...
    try {
      ...
      const errorElement = await driver.findElement(By.css('.swal2-title, .swal2-error, .alert-danger'));
      const errorText = await errorElement.getText();
      
      if (errorText.includes('correo') && (errorText.includes('registrado') || errorText.includes('ERROR'))) {
        console.log('Mensaje de error detectado correctamente: ' + errorText);
        testSteps.push('Mensaje de error confirmado: ' + errorText);
        
        // Cerrar el modal de error si existe
        try {
        ...
        notes = 'Error de validación detectado correctamente. Correo ya registrado: ' + errorText;
        status = 'p';
      } else {
        throw new Error('Mensaje de error no encontrado o incorrecto. Texto: ' + errorText);
      }
    } catch (e) {
      // Verificar si hay mensaje de error en el campo
      try {
        const errorCorreo = await driver.findElement(By.id('textocorreomodal'));
        const errorText = await errorCorreo.getText();
        if (errorText.includes('registrado') || errorText.includes('ERROR')) {
          console.log('Error de validación en campo: ' + errorText);
          notes = 'Error de validación detectado en campo. Correo ya registrado: ' + errorText;
          status = 'p';
        } else {
          throw new Error('No se pudo verificar el error de validación');
        }
      } catch (e2) {
        throw new Error('No se encontró mensaje de error esperado');
      }
    }

    console.log('Prueba de validación completada exitosamente.');
