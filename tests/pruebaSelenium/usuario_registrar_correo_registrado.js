...
const TEST_CASE_EXTERNAL_ID = 'Prueba-94';
...
// === TEST AUTOMATIZADO: REGISTRAR USUARIO - CORREO REGISTRADO ===
async function runTest() {
  ...
  try {
    ...
    // === Paso 8: Verificar mensaje de error "El correo Electronico ya esta registrado" ===
    ...
    try {
      ...
      if (fullText.includes('correo') && (fullText.includes('Electronico') || fullText.includes('Electrónico')) && 
          (fullText.includes('ya esta registrado') || fullText.includes('ya está registrado'))) {
        ...
        notes = 'Correctamente se detectó que el correo ya está registrado. Mensaje: ' + titleText;
        status = 'p';
      } else {
        // Verificar también el alert en la página (id="alertcorreo")
        try {
          const alertCorreo = await driver.findElement(By.id('alertcorreo'));
          const alertStyle = await alertCorreo.getCssValue('display');
          if (alertStyle !== 'none') {
            const alertText = await alertCorreo.getText();
            if (alertText.includes('Correo') && alertText.includes('registrado')) {
              console.log('Alerta de correo registrado encontrada en la página: ' + alertText);
              notes = 'Correctamente se detectó que el correo ya está registrado. Alerta: ' + alertText;
              status = 'p';
            }
          }
        } catch (e) {
      ...
      if (e.message.includes('timeout') || e.message.includes('Unable to locate')) {
        ...
        try {
          ...
          if (alertStyle !== 'none') {
            const alertText = await alertCorreo.getText();
            if (alertText.includes('Correo') && alertText.includes('registrado')) {
              console.log('Alerta de correo registrado encontrada: ' + alertText);
              notes = 'Correctamente se detectó que el correo ya está registrado. Alerta: ' + alertText;
              status = 'p';
            } else {
