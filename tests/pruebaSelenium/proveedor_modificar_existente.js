...
const TEST_CASE_EXTERNAL_ID = 'Prueba-55';
...
// === TEST AUTOMATIZADO: ERROR AL MODIFICAR PROVEEDOR CON IDENTIFICACIÓN YA REGISTRADA ===
async function runTest() {
  ...
  try {
    ...
    // === Paso 3: Obtener tipo y número de documento de otro proveedor existente ===
    testSteps.push('Obtener tipo y número de documento de otro proveedor existente');
    console.log('Buscando otro proveedor existente...');
    
    // Buscar el segundo proveedor en la tabla (si existe) para obtener su tipo y número de documento
    let tipoDocExistente = 'V';
    let numeroDocExistente = '30753995';
    try {
      await driver.wait(until.elementLocated(By.css('tbody tr')), 10000);
      const filas = await driver.findElements(By.css('tbody tr'));
      if (filas.length >= 2) {
        const segundaFila = filas[1];
        const celdas = await segundaFila.findElements(By.css('td'));
        if (celdas.length >= 2) {
          tipoDocExistente = await celdas[0].getText();
          numeroDocExistente = await celdas[1].getText();
          console.log('Otro proveedor existente encontrado: Tipo ' + tipoDocExistente + ', Documento ' + numeroDocExistente);
        }
      }
    } catch (e) {
      console.log('No se pudo obtener otro proveedor existente, usando valores por defecto: Tipo ' + tipoDocExistente + ', Documento ' + numeroDocExistente);
    }

    // === Paso 4: Seleccionar un proveedor y hacer click en Modificar ===
    ...
    // === Paso 6: Modificar con identificación existente ===
    testSteps.push('Modificar con tipo y número de documento existentes de otro proveedor');
    console.log('Modificando con identificación existente...');
    ...
    await driver.executeScript("arguments[0].value = '" + tipoDocExistente + "';", tipoDocSelect);
    await driver.executeScript("arguments[0].dispatchEvent(new Event('change'));", tipoDocSelect);
    await driver.sleep(500);

    // Cambiar número de documento a uno existente
    ...
    await numeroDocInput.sendKeys(numeroDocExistente);
    await driver.sleep(500);

    // === Paso 7: Intentar actualizar ===
    testSteps.push('Hacer click en el botón ACTUALIZAR');
    console.log('Intentando actualizar con identificación existente...');
    ...
    try {
      ...
      if (errorText.includes('existe') || errorText.includes('registrado') || errorText.includes('mismo tipo y numero de documento')) {
        console.log('Mensaje de error detectado correctamente: ' + errorText);
        testSteps.push('Mensaje de error confirmado: ' + errorText);
        notes = 'Error detectado correctamente: ' + errorText + '. Se validó que no se puede modificar un proveedor asignándole una identificación ya registrada.';
        status = 'p';
      } else {
        const htmlContainer = await driver.findElement(By.css('.swal2-html-container'));
        const htmlText = await htmlContainer.getText();
        if (htmlText.includes('existe') || htmlText.includes('registrado') || htmlText.includes('mismo tipo y numero de documento')) {
          console.log('Mensaje de error detectado en HTML: ' + htmlText);
          testSteps.push('Mensaje de error confirmado en HTML: ' + htmlText);
          notes = 'Error detectado correctamente: ' + htmlText + '. Se validó que no se puede modificar un proveedor asignándole una identificación ya registrada.';
          status = 'p';
        } else {
