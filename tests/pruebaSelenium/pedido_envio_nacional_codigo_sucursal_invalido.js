...
const TEST_CASE_EXTERNAL_ID = 'Prueba-106';
...
// === TEST AUTOMATIZADO: CÓDIGO SUCURSAL INVÁLIDO ===
async function runTest() {
  ...
  try {
    ...
    // === Ingresar código de sucursal inválido ===
    testSteps.push('Ingresar código de sucursal inválido');
    console.log('Ingresando código de sucursal inválido (5563)...');
    ...
    await codigoSucursal.sendKeys('5563');
    await driver.sleep(1000);
    ...
    try {
      ...
      if (msgText.includes('Debe tener entre 5 y 7 digitos') || msgText.includes('5 y 7') || msgText.includes('digitos')) {
        console.log('Mensaje de error detectado: ' + msgText);
        testSteps.push('Alerta detectada: Debe tener entre 5 y 7 digitos');
        notes = 'Prueba exitosa. Se detectó correctamente el mensaje de código de sucursal inválido: ' + msgText;
        status = 'p';
      } else {
        throw new Error('Mensaje de error no coincide. Mensaje recibido: ' + msgText);
      }
    } catch (e) {
      const codValue = await codigoSucursal.getAttribute('value');
      if (codValue === '5563' && codValue.length < 5) {
        notes = 'Código inválido ingresado (5563 - menos de 5 dígitos). Validación puede estar en el backend.';
        status = 'p';
      } else {
        throw new Error('No se encontró mensaje de error de código de sucursal inválido');
      }
    }
