...
const TEST_CASE_EXTERNAL_ID = 'Prueba-42';
...
// === TEST AUTOMATIZADO: ERROR AL REGISTRAR DELIVERY CON CARÁCTER INVÁLIDO ===
async function runTest() {
  ...
  try {
    ...
    // === Paso 4: Intentar registrar con caracteres inválidos ===
    testSteps.push('Llenar datos con caracteres inválidos (nombre con número, teléfono con letra)');
    console.log('Llenando datos con caracteres inválidos...');

    // Nombre con número al inicio (0Rhichard Virguez)
    const nombreInput = await driver.findElement(By.id('nombre'));
    await driver.wait(until.elementIsVisible(nombreInput), 10000);
    await nombreInput.clear();
    await nombreInput.sendKeys('0Rhichard Virguez');
    await driver.sleep(500);

    // Teléfono con letra (M0424507195)
    ...
    await contactoInput.sendKeys('M0424507195');
    await driver.sleep(500);
    ...
    await driver.sleep(300);
    await tipoSelect.sendKeys(Key.ARROW_DOWN);
    await driver.sleep(300);
    ...
    console.log('Intentando registrar con caracteres inválidos...');
    ...
    try {
      ...
      const errorContacto = await driver.findElement(By.id('scontacto'));
      const errorNombreText = await errorNombre.getText();
      const errorContactoText = await errorContacto.getText();
      ...
      if (errorContactoText && errorContactoText.length > 0) {
        console.log('Mensaje de error en contacto detectado: ' + errorContactoText);
        testSteps.push('Mensaje de error en contacto: ' + errorContactoText);
      }
      
      if (errorNombreText || errorContactoText) {
        notes = 'Error de validación detectado correctamente. Nombre: ' + errorNombreText + ', Contacto: ' + errorContactoText + '. Se validó que no se pueden usar caracteres inválidos.';
        status = 'p';
      } else {
        ...
        try {
          ...
          if (alertText.includes('obligatorios') || alertText.includes('error')) {
          ...
          throw new Error('No se encontraron mensajes de error de validación');
        }
      }
    } catch (e) {
      try {
    ...
    console.log('Error de validación de caracteres inválidos verificado exitosamente.');
