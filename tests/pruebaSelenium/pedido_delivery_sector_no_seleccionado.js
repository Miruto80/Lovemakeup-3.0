...
const TEST_CASE_EXTERNAL_ID = 'Prueba-110';
...
// === TEST AUTOMATIZADO: SECTOR NO SELECCIONADO ===
async function runTest() {
  ...
  try {
    ...
    // Seleccionar zona y parroquia pero NO sector
    ...
    try {
      ...
      if (msgText.includes('sector') || msgText.includes('Seleccione un') || msgText.includes('Seleccione')) {
        notes = 'Prueba exitosa. Se detectó mensaje de sector no seleccionado: ' + msgText;
        status = 'p';
      } else {
        throw new Error('Mensaje de error no coincide: ' + msgText);
      }
    } catch (e) {
      throw new Error('No se encontró mensaje de error de sector no seleccionado');
    }
