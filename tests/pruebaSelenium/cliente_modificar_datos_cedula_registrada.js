...
const TEST_CASE_EXTERNAL_ID = 'Prueba-77';
...
// === TEST AUTOMATIZADO: MODIFICAR DATOS DEL CLIENTE - CÉDULA REGISTRADA ===
async function runTest() {
  ...
  try {
    ...
    // === Paso 5: Modificar cédula a una ya registrada ===
    testSteps.push('Modificar cédula a una ya registrada');
    console.log('Modificando cédula...');
    ...
    await cedulaInput.sendKeys('10200300'); // Cédula ya registrada
    await driver.sleep(500);
    ...
    testSteps.push('Verificar mensaje de error de cédula registrada');
    ...
    try {
      ...
      if (errorText.includes('cedula') && (errorText.includes('registrada') || errorText.includes('ERROR'))) {
        ...
        notes = 'Error de validación detectado correctamente. Cédula ya registrada: ' + errorText;
        status = 'p';
      } else {
      ...
      try {
        const errorCedula = await driver.findElement(By.id('textocedulamodal'));
        const errorText = await errorCedula.getText();
        if (errorText.includes('registrada') || errorText.includes('ERROR')) {
          console.log('Error de validación en campo: ' + errorText);
          notes = 'Error de validación detectado en campo. Cédula ya registrada: ' + errorText;
          status = 'p';
        } else {
