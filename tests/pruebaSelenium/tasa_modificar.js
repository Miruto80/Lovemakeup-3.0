...
const TEST_CASE_EXTERNAL_ID = 'Prueba-88';
...
// === TEST AUTOMATIZADO: MODIFICAR TASA ===
async function runTest() {
  ...
  try {
    ...
    // Verificar modal SweetAlert "Actualizar Tasa"
    try {
      ...
      if (titleText.includes('Actualizar') || titleText.includes('Tasa')) {
        ...
        testSteps.push('Modal Actualizar Tasa encontrado');
      }
    } catch (e) {
      console.log('No se encontró modal de actualización, continuando...');
    }

    // === Paso 6: Introducir tasa 400.50 ===
    testSteps.push('Introducir tasa 400.50');
    ...
    try {
      ...
      console.log('Tasa ingresada: 400.50');
    ...
    // === Paso 8: Hacer clic en SI ===
    testSteps.push('Hacer clic en SI');
    ...
    try {
      ...
      if (alertText.includes('Modificado') && (alertText.includes('éxito') || alertText.includes('exito'))) {
        await alert.accept();
        await driver.sleep(2000);
        console.log('Mensaje de éxito confirmado: ' + alertText);
        testSteps.push('Mensaje de éxito: ' + alertText);
        notes = 'Tasa modificada exitosamente. Alerta: ' + alertText;
        status = 'p';
      } else {
        await alert.accept();
      ...
      if (e.message.includes('timeout') || e.message.includes('no such alert')) {
        ...
        try {
          ...
          if (successText.includes('Modificado') || successText.includes('exito') || successText.includes('éxito')) {
            console.log('Mensaje de éxito encontrado en la página: ' + successText);
            notes = 'Tasa modificada exitosamente. Mensaje: ' + successText;
            status = 'p';
