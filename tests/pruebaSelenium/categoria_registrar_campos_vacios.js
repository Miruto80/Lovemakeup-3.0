...
const TEST_CASE_EXTERNAL_ID = 'Prueba-70';
...
// === TEST AUTOMATIZADO: ERROR AL REGISTRAR CATEGORIA CON INPUTS VACÍOS ===
async function runTest() {
  ...
  try {
    ...
    // === Paso 4: No llenar los datos (dejar campos vacíos) ===
    testSteps.push('No ingresar datos (dejar campo nombre vacío)');
    console.log('Dejando campos vacíos...');
    ...
    console.log('Intentando registrar con campos vacíos...');
    ...
    try {
      // Verificar que hay mensaje de error en el campo
      ...
      if (errorNombreText && errorNombreText.length > 0) {
        ...
        notes = 'Error de validación detectado correctamente. Nombre: ' + errorNombreText + '. Se validó que el campo debe rellenarse obligatoriamente.';
        status = 'p';
      } else {
        // Verificar si hay un mensaje de alerta general
        try {
          await driver.wait(until.elementLocated(By.css('.swal2-info, .swal2-title, .swal2-error')), 5000);
          const alertElement = await driver.findElement(By.css('.swal2-title'));
          const alertText = await alertElement.getText();
          if (alertText.includes('obligatorios') || alertText.includes('error') || alertText.includes('invalidos') || alertText.includes('Complete todos los campos')) {
            console.log('Mensaje de alerta detectado: ' + alertText);
            notes = 'Error de validación detectado: ' + alertText + '. Se validó que los campos deben rellenarse obligatoriamente.';
            status = 'p';
          } else {
