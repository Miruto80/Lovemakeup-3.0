...
const TEST_CASE_EXTERNAL_ID = 'Prueba-31';
...
// === TEST AUTOMATIZADO: ERROR AL REGISTRAR TIPO USUARIO EXISTENTE ===
async function runTest() {
  ...
  try {
    ...
    // === Paso 4: Llenar formulario con nombre existente (Administrador) ===
    testSteps.push('Llenar datos con nombre existente: Administrador');
    console.log('Llenando datos con nombre existente...');

    // Nombre del Tipo (usar "Administrador" que ya existe)
    ...
    console.log('Intentando registrar tipo usuario existente...');
    ...
    try {
      // Buscar mensaje de error en SweetAlert
      ...
      if (errorText.includes('Administrador') && (errorText.includes('existe') || errorText.includes('registrado'))) {
        ...
        notes = 'Error detectado correctamente: ' + errorText + '. Se validó que no se puede registrar un tipo usuario con nombre existente.';
        status = 'p';
      } else {
        // Verificar también en el contenido HTML
        ...
        if (htmlText.includes('Administrador') && (htmlText.includes('existe') || htmlText.includes('registrado'))) {
          ...
          notes = 'Error detectado correctamente: ' + htmlText + '. Se validó que no se puede registrar un tipo usuario con nombre existente.';
          status = 'p';
        } else {
      ...
      // Verificar si hay algún mensaje de éxito (no debería haberlo)
      try {
    ...
    console.log('Error al registrar tipo usuario existente verificado exitosamente.');
