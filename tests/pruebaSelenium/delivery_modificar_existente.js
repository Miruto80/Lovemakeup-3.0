...
const TEST_CASE_EXTERNAL_ID = 'Prueba-41';
...
// === TEST AUTOMATIZADO: ERROR AL MODIFICAR DELIVERY CON NOMBRE EXISTENTE ===
async function runTest() {
  ...
  try {
    ...
    // === Paso 3: Obtener nombre de un delivery existente ===
    testSteps.push('Obtener nombre de un delivery existente');
    console.log('Buscando delivery existente...');
    
    let nombreExistente = 'Rhichard Virguez';
    try {
      await driver.wait(until.elementLocated(By.css('tbody tr')), 10000);
      const filas = await driver.findElements(By.css('tbody tr'));
      if (filas.length > 1) {
        // Obtener el nombre del segundo delivery (no el que vamos a modificar)
        const segundaFila = filas[1];
        const celdas = await segundaFila.findElements(By.css('td'));
        if (celdas.length > 0) {
          nombreExistente = await celdas[0].getText();
          console.log('Nombre de delivery existente encontrado: ' + nombreExistente);
        }
      }
    } catch (e) {
    ...
    // === Paso 4: Seleccionar un delivery y hacer click en Modificar ===
    ...
    // === Paso 6: Modificar el nombre a uno existente ===
    testSteps.push('Modificar el nombre a uno existente: ' + nombreExistente);
    console.log('Modificando nombre a ' + nombreExistente + '...');
    ...
    // === Paso 7: Hacer click en ACTUALIZAR ===
    ...
    try {
      ...
      if (errorText.includes('existe') || errorText.includes('registrado') || errorText.includes('mismo nombre')) {
        ...
        notes = 'Error detectado correctamente: ' + errorText + '. Se validó que no se puede modificar un delivery asignándole un nombre existente.';
        status = 'p';
      } else {
        ...
        if (htmlText.includes('existe') || htmlText.includes('registrado') || htmlText.includes('mismo nombre')) {
          ...
          notes = 'Error detectado correctamente: ' + htmlText + '. Se validó que no se puede modificar un delivery asignándole un nombre existente.';
          status = 'p';
        } else {
    ...
    console.log('Error al modificar delivery con nombre existente verificado exitosamente.');
