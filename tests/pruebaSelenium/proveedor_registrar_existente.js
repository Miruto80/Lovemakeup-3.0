...
const TEST_CASE_EXTERNAL_ID = 'Prueba-54';
...
async function runTest() {
  ...
  try {
    ...
    // === Paso 3: Obtener tipo y número de documento de un proveedor existente ===
    testSteps.push('Obtener tipo y número de documento de un proveedor existente');
    console.log('Buscando proveedor existente...');
    
    // Buscar el primer proveedor en la tabla para obtener su tipo y número de documento
    ...
    try {
      ...
      if (celdas.length >= 2) {
        ...
        console.log('Proveedor existente encontrado: Tipo ' + tipoDocExistente + ', Documento ' + numeroDocExistente);
      }
    } catch (e) {
      console.log('No se pudo obtener proveedor existente, usando valores por defecto: Tipo ' + tipoDocExistente + ', Documento ' + numeroDocExistente);
    }
    ...
    // === Paso 5: Llenar formulario con datos existentes ===
    testSteps.push('Llenar datos con tipo y número de documento existentes');
    console.log('Llenando datos con información existente...');
    ...
    console.log('Intentando registrar proveedor existente...');
    ...
    try {
      ...
      if (errorText.includes('existe') || errorText.includes('registrado') || errorText.includes('mismo tipo y numero de documento')) {
        ...
        notes = 'Error detectado correctamente: ' + errorText + '. Se validó que no se puede registrar un proveedor con el mismo tipo y número de documento.';
        status = 'p';
      } else {
        ...
        if (htmlText.includes('existe') || htmlText.includes('registrado') || htmlText.includes('mismo tipo y numero de documento')) {
          ...
          notes = 'Error detectado correctamente: ' + htmlText + '. Se validó que no se puede registrar un proveedor con el mismo tipo y número de documento.';
          status = 'p';
        } else {
      ...
      try {
        ...
        if (errorText.includes('existe') || errorText.includes('registrado')) {
          ...
          notes = 'Error detectado correctamente: ' + errorText + '. Se validó que no se puede registrar un proveedor existente.';
          status = 'p';
        } else {
