...
const TEST_CASE_EXTERNAL_ID = 'Prueba-13';
...
// === TEST AUTOMATIZADO: REGISTRAR COMPRA QUE SUPERA EL STOCK MÁXIMO PERMITIDO ===
async function runTest() {
  ...
  try {
    ...
    testSteps.push('Abrir el formulario de compra');
    ...
    // === Paso 4: Seleccionar proveedor y producto ===
    testSteps.push('Seleccionar proveedor y producto');
    ...
    // Obtener información del producto (stock actual y máximo)
    const productoInfo = await driver.executeScript(`
      ...
      for (var i = 1; i < options.length; i++) {
        ...
        if (option.value && option.value !== '') {
          ...
          var stockMaximo = option.getAttribute('data-stock-maximo');
          return {
            value: option.value,
            text: option.text,
            stockActual: stockActual ? parseFloat(stockActual) : null,
    ...
    console.log('Producto encontrado: ' + productoInfo.text);
    console.log('Stock actual: ' + productoInfo.stockActual);
    console.log('Stock máximo: ' + productoInfo.stockMaximo);
    
    // Seleccionar el producto
    await driver.executeScript(`
      var select = arguments[0];
      var productValue = arguments[1];
      for (var i = 1; i < select.options.length; i++) {
        if (select.options[i].value === productValue) {
          select.selectedIndex = i;
          if (typeof $ !== 'undefined' && $(select).hasClass('select2-hidden-accessible')) {
            $(select).val(productValue).trigger('change');
          } else {
    ...
    `, productoSelect, productoInfo.value);
    
    await driver.sleep(1000);

    // === Paso 5: Ingresar cantidad superior al stock máximo ===
    testSteps.push('Ingresar una cantidad superior al stock máximo del producto');
    console.log('Ingresando cantidad que supera el stock máximo...');
    ...
    // Calcular cantidad que supera el stock máximo
    let cantidadExcedente = 1000; // Valor por defecto alto
    if (productoInfo.stockMaximo !== null && productoInfo.stockActual !== null) {
      cantidadExcedente = Math.ceil(productoInfo.stockMaximo - productoInfo.stockActual + 1);
    }
    
    await cantidadInput.sendKeys(cantidadExcedente.toString());
    ...
    console.log('Intentando registrar compra con cantidad excedente...');
    ...
    // === Paso 7: Verificar error de stock máximo ===
    testSteps.push('Verificar que el sistema muestra mensaje de error de stock máximo');
    ...
    try {
      ...
      if (modalVisible && errorEncontrado) {
        ...
        if (errorLower.includes('stock') || errorLower.includes('máximo') || errorLower.includes('maximo') || errorLower.includes('superaría')) {
          console.log('Test exitoso: El sistema detecto el exceso de stock');
          notes = 'El sistema correctamente detecto que la cantidad supera el stock máximo. Mensaje: ' + errorText;
          status = 'p';
        } else {
          throw new Error('El mensaje de error no es relacionado con stock máximo: ' + errorText);
        }
      } else if (!modalVisible && !errorEncontrado) {
        throw new Error('El sistema guardo la compra sin validar el stock máximo');
      } else {
        throw new Error('No se pudo verificar el comportamiento esperado');
      }
    } catch (e) {
      if (e.message && !e.message.includes('Test exitoso')) {
        throw e;
      }
    }
