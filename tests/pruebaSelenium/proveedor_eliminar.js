...
const TEST_CASE_EXTERNAL_ID = 'Prueba-53';
...
async function runTest() {
  ...
  try {
    ...
    await driver.wait(until.elementLocated(By.xpath("//button[contains(@onclick, 'eliminarProveedor')]")), 15000);
    console.log('Modulo Proveedor cargado correctamente.');
    testSteps.push('Módulo Proveedor cargado correctamente');

    // === Paso 3: Seleccionar un proveedor y hacer click en Eliminar ===
    testSteps.push('Seleccionar un proveedor y hacer click en Eliminar');
    console.log('Buscando botón de eliminar...');
    
    const eliminarButtons = await driver.findElements(By.xpath("//button[contains(@onclick, 'eliminarProveedor')]"));
    ...
    try {
      ...
      if (titleText.includes('Eliminar proveedor') || titleText.includes('eliminar')) {
    ...
    try {
      ...
      if (successText.includes('Proveedor eliminado') || successText.includes('éxito')) {
      ...
      try {
        ...
        if (await tabla.isDisplayed()) {
          console.log('Tabla visible - Proveedor eliminado exitosamente.');
        } else {
    ...
    notes = 'Proveedor eliminado exitosamente. Se mostró aviso de confirmación y se confirmó la eliminación.';
    status = 'p';
