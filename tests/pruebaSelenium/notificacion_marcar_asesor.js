...
const TEST_CASE_EXTERNAL_ID = 'Prueba-50'; 
...
// === TEST AUTOMATIZADO: MARCAR NOTIFICACIÓN COMO LEÍDA POR EL ASESOR DE VENTAS ===
async function runTest() {
  ...
  try {
    ...
    // Buscar el primer botón de "Marcar como leída" para asesor de ventas
    const marcarLeidaBtn = await driver.findElement(By.css('button.btn-action[data-accion="marcarLeidaAsesora"]'));
    ...
    // === Paso 6: Verificar que la notificación desaparece para el asesor ===
    testSteps.push('Verificar que la notificacion desaparece para el asesor');
    console.log('Verificando que la notificacion desaparece para el asesor...');
    ...
    try {
      ...
      if (actionText.includes('Actualizado') || actionText.includes('Leída')) {
        ...
        testSteps.push('Notificacion marcada como leida correctamente para el asesor');
      } else {
        ...
        if (estadoText.includes('Leída por asesora') || estadoText.includes('Leída')) {
      ...
      // Si no se encuentra el elemento, puede ser que la notificación desapareció completamente para el asesor
      console.log('Verificando si la notificacion desaparecio completamente para el asesor...');
      const notifications = await driver.findElements(By.css('button.btn-action[data-accion="marcarLeidaAsesora"]'));
      if (notifications.length === 0) {
        console.log('Todas las notificaciones fueron marcadas como leidas por el asesor.');
        testSteps.push('Todas las notificaciones fueron marcadas como leidas por el asesor');
      } else {
        throw new Error('La notificacion no desaparecio correctamente para el asesor');
      }
    }

    console.log('Notificacion marcada como leida exitosamente por el asesor de ventas.');
    notes = 'Notificacion marcada como leida exitosamente por el asesor de ventas. La notificacion desaparece para el asesor de ventas pero no para el administrador hasta que este la marque como leida.';
    status = 'p';
