...
const TEST_CASE_EXTERNAL_ID = 'Prueba-105';
...
// === TEST AUTOMATIZADO: EMPRESA NO SELECCIONADA ===
async function runTest() {
  ...
  try {
    ...
    // === NO seleccionar Empresa ===
    testSteps.push('No seleccionar Empresa');
    console.log('Omitiendo selección de empresa...');
    ...
    // No seleccionamos ninguna opción
    ...
    // Intentar continuar sin seleccionar empresa
    ...
    try {
      ...
      if (msgText.includes('Falto Empresa') || msgText.includes('Empresa por seleccionar') || msgText.includes('seleccione')) {
        console.log('Mensaje de error detectado: ' + msgText);
        testSteps.push('Alerta detectada: Falto Empresa por seleccionar');
        notes = 'Prueba exitosa. Se detectó correctamente el mensaje de empresa no seleccionada: ' + msgText;
        status = 'p';
      } else {
        ...
        try {
          ...
          if (msgText2.includes('Falto Empresa') || msgText2.includes('Empresa')) {
            notes = 'Prueba exitosa. Se detectó mensaje de empresa no seleccionada: ' + msgText2;
            status = 'p';
          } else {
            throw new Error('Mensaje de error no coincide. Mensaje recibido: ' + msgText2);
          }
        } catch (e2) {
          throw new Error('No se encontró mensaje de error de empresa no seleccionada');
        }
      }
    } catch (e) {
      throw new Error('No se encontró mensaje de error de empresa no seleccionada');
