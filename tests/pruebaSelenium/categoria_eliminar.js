...
const TEST_CASE_EXTERNAL_ID = 'Prueba-63';
...
// === TEST AUTOMATIZADO: ELIMINAR CATEGORIA ===
async function runTest() {
  ...
  try {
    ...
    // === Paso 3: Seleccionar una categoria y hacer click en Eliminar ===
    testSteps.push('Seleccionar una categoria y hacer click en Eliminar');
    ...
    try {
      ...
      if (titleText.includes('Eliminar categoria') || titleText.includes('eliminar')) {
    ...
    try {
      ...
      if (successText.includes('Categoria eliminada') || successText.includes('éxito')) {
      ...
      try {
        ...
        if (await tabla.isDisplayed()) {
          console.log('Tabla visible - Categoria eliminada exitosamente.');
        } else {
    ...
    notes = 'Categoria eliminada exitosamente. Se mostró aviso de confirmación y se confirmó la eliminación.';
    status = 'p';
