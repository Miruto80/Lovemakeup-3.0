...
const TEST_CASE_EXTERNAL_ID = 'Prueba-59';
...
// === TEST AUTOMATIZADO: ERROR AL MODIFICAR PROVEEDOR POR INCUMPLIMIENTO DEL RANGO PERMITIDO DE CARACTERES ===
async function runTest() {
  ...
  try {
    ...
    testSteps.push('Modificar con valores fuera de rango (nombre muy corto: "Rh", teléfono muy corto: "04245", correo sin @)');
    ...
    try {
      ...
      if (errorNombreText || errorCorreoText || errorTelefonoText || errorDireccionText) {
        notes = 'Error de validación detectado correctamente. Nombre: ' + errorNombreText + ', Correo: ' + errorCorreoText + ', Teléfono: ' + errorTelefonoText + ', Dirección: ' + errorDireccionText + '. Se validó que se debe cumplir el rango permitido de caracteres.';
        status = 'p';
      } else {
        try {
