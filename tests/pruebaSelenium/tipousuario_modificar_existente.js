...
const TEST_CASE_EXTERNAL_ID = 'Prueba-32';
...
// === TEST AUTOMATIZADO: ERROR AL MODIFICAR TIPO USUARIO CON NOMBRE EXISTENTE ===
async function runTest() {
  ...
  try {
    ...
    // Buscar el primer botón de modificar que no sea el Administrador (id=3)
    ...
    // === Paso 5: Modificar el nombre a "Administrador" (que ya existe) ===
    testSteps.push('Modificar el nombre a "Administrador" (nombre existente)');
    console.log('Modificando nombre a Administrador...');
    ...
    await nombreInput.sendKeys('Administrador');
    await driver.sleep(500);

    // === Paso 6: Hacer click en ACTUALIZAR ===
    testSteps.push('Hacer click en el botón ACTUALIZAR');
    console.log('Intentando actualizar con nombre existente...');
    ...
    try {
      ...
      if (errorText.includes('Administrador') && (errorText.includes('existe') || errorText.includes('registrado'))) {
        console.log('Mensaje de error detectado correctamente: ' + errorText);
        testSteps.push('Mensaje de error confirmado: ' + errorText);
        notes = 'Error detectado correctamente: ' + errorText + '. Se validó que no se puede modificar un tipo usuario asignándole un nombre existente.';
        status = 'p';
      } else {
        const htmlContainer = await driver.findElement(By.css('.swal2-html-container'));
        const htmlText = await htmlContainer.getText();
        if (htmlText.includes('Administrador') && (htmlText.includes('existe') || htmlText.includes('registrado'))) {
          console.log('Mensaje de error detectado en HTML: ' + htmlText);
          testSteps.push('Mensaje de error confirmado: ' + htmlText);
          notes = 'Error detectado correctamente: ' + htmlText + '. Se validó que no se puede modificar un tipo usuario asignándole un nombre existente.';
          status = 'p';
        } else {
    ...
    console.log('Error al modificar tipo usuario con nombre existente verificado exitosamente.');
